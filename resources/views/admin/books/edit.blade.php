...
<div class="card card-warning">
    ...
    <form action="{{ route('admin.books.update', $book->BookID) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card-body">
            @if ($errors->any())
            ...
            <div class="form-group">
                ...
                <select name="AuthorID" id="AuthorID" class="form-control">
                    ...
                    @foreach($authors as $author)
                        <option value="{{ $author->AuthorID }}" {{ old('AuthorID', $book->AuthorID) == $author->AuthorID ? 'selected' : '' }}>
            ...
            <div class="form-group">
                <label for="NewAuthorFirstName">Або оновіть старого автора</label>
            ...
            <div class="form-group">
                ...
                <textarea name="Description" id="Description" rows="5" class="form-control @error('Description') is-invalid @enderror">{{ old('Description', $book->Description) }}</textarea>
                @error('Description') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="Cover">Нова обкладинка</label>
                <input type="file" name="Cover" id="Cover" class="form-control-file @error('Cover') is-invalid @enderror">
                @error('Cover') <span class="text-danger">{{ $message }}</span> @enderror
                @if($book->Cover)
                    <div class="mt-3">
                        <p class="mb-2">Поточна обкладинка:</p>
                        <img src="{{ asset('covers/' . $book->Cover) }}" alt="{{ $book->Title }}" style="width: 180px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                    </div>
                @endif
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="DailyRentPrice" id="DailyRentPrice" class="form-control @error('DailyRentPrice') is-invalid @enderror" value="{{ old('DailyRentPrice', $book->DailyRentPrice) }}">
                @error('DailyRentPrice') <span class="invalid-feedback">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="PublicationYear">Рік видання</label>
                <input type="number" name="PublicationYear" id="PublicationYear" class="form-control" value="{{ old('PublicationYear', $book->PublicationYear) }}">
            </div>

            <div class="form-group">
                <label for="CopiesAvailable">Доступно примірників</label>
                <input type="number" name="CopiesAvailable" id="CopiesAvailable" class="form-control" value="{{ old('CopiesAvailable', $book->CopiesAvailable) }}">
            </div>

            <div class="form-group">
                <label for="CollateralValue">Заставна вартість (грн)</label>
                <input type="number" step="0.01" name="CollateralValue" id="CollateralValue" class="form-control" value="{{ old('CollateralValue', $book->CollateralValue) }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="GenreID" id="GenreID" class="form-control @error('GenreID') is-invalid @enderror">
                    @foreach($genres as $genre)
                        <option value="{{ $genre->GenreID }}" {{ old('GenreID', $book->GenreID) == $genre->GenreID ? 'selected' : '' }}>{{ $genre->GenreName }}</option>
                    @endforeach
                </select>
